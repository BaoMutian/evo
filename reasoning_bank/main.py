...
def run_single_turn(args, logger):
    ...
    if args.matts == "none":
        ...
        logger.info(f"成功率: {success_count}/{len(task_ids)} ({success_count/len(task_ids):.2%})")
...
def run_multi_turn(args, logger):
    ...
    if env_name == "alfworld":
        ...
        env = ScienceWorldEnv(
            ...
            max_steps=args.max_steps,
            split=args.split if args.split in ["train", "dev", "test"] else "dev",
            seed=args.seed,
        )
    ...
    # 创建 Extractor
    extractor = MemoryExtractor(llm_service=llm) if args.use_memory and not args.no_extract else None
    
    # 运行统计
    total = args.num_tasks if args.num_tasks and args.num_tasks > 0 else len(env)
...
def _run_multiturn_episode(env, llm, memory_bank, task_id, max_steps, temperature, verbose):
    ...
    while not done and step < max_steps:
        ...
        if verbose:
            obs_display = observation[:100] + "..." if len(observation) > 100 else observation
